...
------------------------------------------------------------------------------
Sistem ini dibuat untuk mempermudah proses verifikasi peserta ujian di laboratorium secara otomatis. Alih-alih pengawas harus mengecek tumpukan kertas secara manual, mereka cukup memasukkan nomor kursi untuk melihat siapa peserta yang seharusnya duduk di sana. Sistem ini tidak hanya menampilkan nama, tapi juga memberikan informasi posisi duduk seperti indeks data dan pembagian zona baris (depan atau belakang). Tujuannya agar alur masuk ruang ujian lebih tertib dan meminimalisir kesalahan posisi duduk siswa. sistem ini mengandalkan Binary Search sebagai motor penggeraknya. Karena data nomor kursi sudah tersusun rapi, algoritma ini bisa bekerja sangat cerdik dengan cara membelah data menjadi dua bagian terus-menerus sampai posisi kursi ditemukan. Cara ini jauh lebih ngebut dibanding harus mengecek satu-satu dari awal. Untuk penyimpanan datanya, saya menggunakan List of Tuples. List bertugas menjaga agar urutan data tetap konsisten, sementara Tuple digunakan untuk mengunci pasangan "nomor kursi dan nama" agar datanya aman, efisien, dan tidak mudah berubah secara tidak sengaja.
...
Pada baris 1 Mendefinisikan fungsi yang menerima dua parameter yaitu data (kumpulan data kursi) dan target (nomor kursi yang dicari).
...
Pada baris 4 Melakukan perulangan selama rentang pencarian masih valid yang dimana Jika nomor tidak ditemukan, pembatas low akan bergeser ke kanan melewati high atau high bergeser ke kiri melewati low.
...
Pada baris 8 dan 9 Jika nomor di tengah ternyata lebih kecil dari yang dicari, berarti target ada di sisi kanan Maka low digeser maju melewati tengah (mid + 1).
...
Pada baris 14 Mendefinisikan fungsi utama program.
Pada baris 15 Inisialisasi basis data yang dimana beberapa menyimpan dalam bentuk tuple ke dalam sebuah list.
...
Pada baris 21 penanganan kesalahan untuk mengantisipasi kesalahan input.
...
Pada baris 27 Menentukan zonasi area, Jika indeks berada di 0-3, masuk Baris Depan, jika tidak, masuk Baris Belakang.
Pada baris 29, 30, 31, 32 untuk menampilkan sebuah data yang telah ditemukan ke dalam output
Pada baris 33 dan 34 Jika indeks adalah -1, tampilkan pesan bahwa data kursi tersebut tidak ditemukan atau kosong.
...
Pada baris 38 Memastikan bahwa fungsi main() hanya akan dieksekusi jika file ini dijalankan secara langsung (sebagai program utama), bukan saat diimpor sebagai modul oleh file lain.
